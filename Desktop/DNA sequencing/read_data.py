...
df = pd.read_csv('n=1000_file.csv')
...
plt.plot(x1,y1,label = 'Pi = Pd = 0.008',color='b')
plt.fill_between(x1,y1-0.05*sd1,y1+0.05*sd1,color='cornflowerblue')

plt.plot(x2,y2,label = 'Pi = Pd = 0.01',color='y')
plt.fill_between(x1,y2-0.05*sd2,y2+0.05*sd2,color='khaki')

plt.plot(x3,y3,label = 'Pi = Pd = 0.02',color='r')
plt.fill_between(x3,y3-0.05*sd3,y3+0.05*sd3,color='tomato')


plt.xlabel('Probability of substitution')
plt.ylabel('Error percentage %')
plt.title('n = 100 length sequence')
