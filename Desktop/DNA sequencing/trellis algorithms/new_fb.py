...
for j in range(len(recieved)+1):
    for i in range(len(transmitted)+1):
        ...
        if reverse_graph[node]:
            ...
            alphas[node] = sum([alphas[neighbour]*edges[(neighbour,node)] for neighbour in reverse_graph[node]])
...
for j in range(len(recieved)+1)[::-1]:
    for i in range(len(transmitted)+1)[::-1]:
        ...
        if my_graph[node]:
            ...
            betas[node] = sum([betas[neighbour]*edges[((node,neighbour))] for neighbour in my_graph[node]])
...
if __name__ == '__main__':
    ...
    draw_trellis(my_graph,tuples,recieved,transmitted,edges,alphas,betas,values,transitions,name='newfb.html',directed=True)
